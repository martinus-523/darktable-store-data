...
local utils = require "darktableluascripts/lib/utilities"
...
local function parse_tags(output)
    local tags = {}

    -- Capture everything after "TAGS:"
    local tags_section = output:match("TAGS:%s*(.+)")
    if tags_section then
        for tag in tags_section:gmatch("([^,]+)") do
...
--[[
  Generates tags for a given JPEG image using an AI language model.
  ...
  @return (table): The parsed result containing tags generated by the AI.
]]
...
local function generate_and_attach(images)
    ...
    for idx, img in ipairs(images) do
        ...
        local tags, caption = generate_tags_with_ai(jpegfile)
